...
// upon exit, deinits and turns off all cameras.
//
//
//
#include "Spinnaker.h"
...
chrono::time_point<chrono::system_clock, chrono::duration<double>> EXPERIMENT_START = std::chrono::system_clock::now();
...
// takes in a pointer to a USB hub and enables or disables all the ports on the Hub as required. 
int toggleUSBPorts(aUSBHub2x4* hubptr, portStatus status) {
	...
	if (status == ENABLE) {
		for (int i = 0; i < 4; ++i) {
			err = (*hubptr).usb.setPortEnable(i);
			cout << "    Port: " << i << " Error: " << err << endl;
			result |= err;
		}
	}
	if (status == DISABLE) {
		for (int i = 0; i < 4; ++i) {
			err = (*hubptr).usb.setPortDisable(i);
			cout << "    Port: " << i << " Error: " << err << endl;
			result |= err;
		}
	}
...
int main(int argc, char** argv)
{
	...
	// check for a cmd line argument, if not, exit immediately
	string rawFile; 
	...
	if (argc >= 2) {
		...
		cout << "No trigger config file given; please enter a config file and try again" << endl;
		return -1;
	}
	...
	// create a USB Hub Instance 
	aUSBHub2x4 hub; 
	aErr err = aErrNone;

	// NOTE: linkType:: namespace is required, otherwise the brainstem library gets confused with Spinnaker, which also has an object called "USB"
	err = hub.discoverAndConnect(linkType::USB); 
	if (err != aErrNone) {
		cout << "Error " << err << " encountered connecting to USB Hub" << endl;
		return -1; 
	}
	else {
		cout << "Connected to USB Hub." << endl;

	}

	errorFlag |= toggleUSBPorts(&hub, ENABLE);

	if (errorFlag != 0) {
		toggleUSBPorts(&hub, DISABLE);
		return -1;
	}

	// sleep to allow time for cameras to turn on, become recognized by system
	aTime_MSSleep(5000);
	...
	chrono::time_point<chrono::system_clock, chrono::duration<double>> experimentEnd = EXPERIMENT_START + chrono::minutes(configJson["RunUntil"]);
	...
	while (true) { 
		...
		this_thread::sleep_for(chrono::milliseconds(configJson["TriggerTiming"]));
		...
		if ((errorFlag != 0 && configJson["ExitOnError"]) || (chrono::system_clock::now() > experimentEnd)) {
